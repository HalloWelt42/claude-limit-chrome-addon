# Claude Dashboard – Datenschutzerklärung / Privacy Policy
...
Claude Dashboard ist eine Browser-Erweiterung, die **ausschließlich lokal** auf deinem Gerät arbeitet. Es werden **keine Daten an externe Server übertragen**, keine Analysen durchgeführt und kein Tracking eingesetzt.
...
Claude Dashboard führt folgende Netzwerkanfragen durch – **automatisch alle 5 Minuten**:
...
Claude Dashboard verwendet **keine** Analyse-, Werbe- oder Tracking-Dienste. Die einzigen externen Anfragen gehen an claude.ai (deine eigenen Kontodaten) und status.claude.com (öffentlicher Status).
...
Claude Dashboard is a browser extension that operates **entirely locally** on your device. **No data is transmitted to external servers**, no analytics are collected, and no tracking is used.
...
Claude Dashboard makes the following network requests – **automatically every 5 minutes**:
...
Claude Dashboard uses **no** analytics, advertising, or tracking services. The only external requests go to claude.ai (your own account data) and status.claude.com (public status).
